...
app.post('/api/admin/issue-certificate', upload.fields([{ name: 'pdfFile', maxCount: 1 }, { name: 'studentPhoto', maxCount: 1 }]), async (req, res) => {
    ...
    try {
        ...
        // Build the transaction object with gasPrice and convert BigInts to strings
        const tx = {
            from: ADMIN_WALLET,
            to: CONTRACT_ADDRESS,
            data: txData,
...
app.get('/api/student/certificates/:rollNumber', async (req, res) => {
    ...
    try {
        ...
        const safeCertificates = (studentRecord.certificates || []).map(cert => ({
            id: cert.certificateHash, // FIX: Renamed 'certificateHash' to 'id'
            name: studentRecord.studentName, // FIX: Renamed 'studentName' to 'name'
